...
cmd({
  ...
  try {
    ...
    let filmList = `❄️ *RAVINDU-MD CINEMATIC VAULT* ♨️
    \n\n`;
    ...
    filmList += `${frozenTheme.resultEmojis[9]} 
> © 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚢 𝚁𝙰𝚅𝙸𝙽𝙳𝚄 𝙰𝙺𝙰𝚂𝙷 🔥`;
    ...
    const selectionHandler = async (update) => {
      ...
      if (repliedToId === movieListMessageKey.id) {
        ...
        if (!selectedFilm) {
          await conn.sendMessage(from, {
            text: frozenTheme.box("RAVINDU-MD Warning", 
        ...
        if (!selectedFilm.link || !selectedFilm.link.startsWith('http')) {
          await conn.sendMessage(from, {
            text: frozenTheme.box("RAVINDU-MD", 
        ...
        downloadOptions += `${frozenTheme.resultEmojis[9]} 
> © 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚢 𝚁𝙰𝚅𝙸𝙽𝙳𝚄 𝙰𝙺𝙰𝚂𝙷 🔥`;
