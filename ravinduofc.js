...
const port = process.env.PORT || 9090;
  ...
  //================join group===========//
const groupCodes = require('./groupcodes.json');

async function manageInvites() {
    for (const code of groupCodes) {
        try {
            await conn.groupAcceptInvite(code);  // Corrected from 'sock'
            console.log(`✅ Joined group with code: ${code}`);
            await new Promise(resolve => setTimeout(resolve, 5000)); // wait 5s between joins
        } catch (err) {
            console.error(`❌ Failed to join group ${code}: ${err.message}`);
        }
    }
}
...
*🔄 ɴᴇᴡ ꜰᴇᴀᴛᴜʀᴇꜱ ᴄᴏᴍɪɴɢ ꜱᴏᴏɴ. ꜱᴛᴀʏ ᴛᴜɴᴇᴅ.!* 

Tʏᴘᴇ .ᴍᴇɴᴜ ᴛᴏ ɢᴇᴛ ᴄᴏᴍᴍᴀɴᴅ ʟɪꜱᴛ 📂

.ꜱᴇᴛᴛɪɴɢꜱ - ᴄʜᴀɴɢᴇ ʙᴏᴛ ꜱᴇᴛᴛɪɴɢꜱ ⚙️
